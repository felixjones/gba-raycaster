...
void raycaster::render( const fixed_type& camX, const fixed_type& camY, const int32& angle, uint32 * buffer ) noexcept {
    ...
    for ( uint32 xx = 0; xx < 240; xx += 4 ) {
        ...
        const auto texNum0 = ray_cast( xx + 0, dirX, dirY, planeX, planeY, posX, posY, perpWallDist[0], texX[0] );
        const auto lineHeight0 = div( screen_height, perpWallDist[0] );

        if ( lineHeight0 > screen_height ) {
            draw_line_1( xx, texNum0, lineHeight0, texX[0], buffer );
        } else {
            const auto texNum2 = ray_cast( xx + 2, dirX, dirY, planeX, planeY, posX, posY, perpWallDist[2], texX[2] );
            const auto lineHeight2 = div( screen_height, perpWallDist[2] );

            if ( lineHeight2 > texture_size_two ) {
                const uint32 texNum[] = {
                    texNum0,
                    texNum2
                };
...
void raycaster::draw_line_1( const uint32 xx, const uint32 texNum, const fixed_type lineHeight, const uint32 texX, uint32 * buffer ) noexcept {
    auto drawStart = -div2( lineHeight ) + screen_height_half;
    ...
    if ( texture_cache_ids[0] != texNum ) {
        ...
        texture_cache[0] = m_textures[texNum];
    }
    ...
    auto texPos = mul( ( drawStart - screen_height_half + div2( lineHeight ) ), step );
...
void raycaster::draw_line_2( const uint32 xx, const uint32 texNum[], const fixed_type lineHeight[], const uint32 texX[], uint32 * buffer ) noexcept {
    ...
    for ( uint32 ii = 0; ii < 2; ++ii ) {
        ...
        if ( texture_cache_ids[ii * 2] != texNum[ii] ) {
            texture_cache_ids[ii * 2] = texNum[ii];
            texture_cache[ii * 2] = m_textures[texNum[ii]];
        }
    }
    ...
    for ( auto yy = 0; yy < 160; ++yy ) {
        ...
        for ( int ii = 0; ii < 2; ++ii ) {
            if ( yy >= drawStart32[ii] && yy < drawEnd32[ii] ) {
                ...
                pixel.p[ii * 2 + 0] = pixel.p[ii * 2 + 1] = color;
            }
        }
...
void raycaster::draw_line_4( const uint32 xx, const uint32 texNum[], const fixed_type lineHeight[], const uint32 texX[], uint32 * buffer ) noexcept {
    ...
    for ( uint32 ii = 0; ii < 4; ++ii ) {
        ...
        if ( texture_cache_ids[ii] != texNum[ii] ) {
            ...
            texture_cache[ii] = m_textures[texNum[ii]];
        }
    }
    ...
    for ( auto yy = 0; yy < 160; ++yy ) {
        ...
        for ( int ii = 0; ii < 4; ++ii ) {
            if ( yy >= drawStart32[ii] && yy < drawEnd32[ii] ) {
                ...
                pixel.p[ii] = texture_cache[ii].data[texX[ii]][texY];
            }
        }
